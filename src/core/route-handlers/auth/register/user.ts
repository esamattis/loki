...
export async function createRegistrationUser(
    ...
    if (firstUserUuid)
        return { uuid: firstUserUuid, consumedInvitation: false };
...
export async function compensateRegistration(
    ...
    registration: {
        userUuid: string;
        invitationCode?: string;
        consumedInvitation: boolean;
    },
): Promise<void> {
    if (registration.consumedInvitation && registration.invitationCode) {
        await db.batch([
            db.delete(users).where(eq(users.uuid, registration.userUuid)),
            db
                .update(invitations)
                .set({ count: sql`${invitations.count} + 1` })
                .where(eq(invitations.code, registration.invitationCode)),
        ]);
        return;
    }
    await db.batch([
