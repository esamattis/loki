...
export async function createAircraft(c: AppRequestContext) {
...
export async function getAircraftListResponse(c: AppRequestContext) {
...
export async function getEditAircraftResponse(
    c: AppRequestContext,
    dangerError?: string,
) {
    const db = getAppContext(c).db;
...
async function mergeAircraft(
    ...
    if (!targetUuid || targetUuid === sourceUuid) {
        return getEditAircraftResponse(
            c,
...
export async function updateAircraft(c: AppRequestContext) {
    const db = getAppContext(c).db;
    const userUuid = getAppContext(c).getUser().uuid;
    const { uuid } = routes.logbook.aircraft.edit.params(c);
    if (!uuid) {
...
export function getNewAircraftResponse(c: AppRequestContext) {
    return c.render(
