---
name: sync-core
description: Sync src/core into the current project from another project that uses the same core. Use when the user asks to sync, copy, or update the shared core and explicitly names the source project.
---

# Sync shared core

Sync `src/core` from another project into the current project. The current
workspace is always the destination; the project named by the user is always
the source.
...
- Resolve and state the source and destination roots before copying. They must
  be different Git worktrees and both must contain `src/core`.
...
Compare the actual core trees, including uncommitted and untracked files:
...
- Keep the operation scoped to `src/core` unless the user explicitly requests
  related files outside it.
